...
func TestUpdateConnection_ChangesPersist(t *testing.T) {
	db := openTestDB(t)
	...
	created.Host = "10.0.0.5"
	created.Port = 5433
	created.Username = strPtr("newuser")
	created.ParamsJSON = `{"sslmode":"disable"}`

	updated, err := UpdateConnection(db, created)
	if err != nil {
		t.Fatalf("UpdateConnection failed: %v", err)
	}
	if updated.Name != "after-update" || updated.Host != "10.0.0.5" || updated.Port != 5433 {
	...
	if fetched.ParamsJSON != `{"sslmode":"disable"}` {
		t.Errorf("expected ParamsJSON update to persist, got %q", fetched.ParamsJSON)
	}
}
...
func TestUpdateConnection_NotFound(t *testing.T) {
	...
	ghost := &Connection{ID: 999999, Name: "ghost", Engine: EnginePostgres, Host: "localhost", Port: 5432}
	if _, err := UpdateConnection(db, ghost); err == nil {
		t.Fatal("expected UpdateConnection on a non-existent ID to return an error")
	}
}
